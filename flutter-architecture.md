...
## 九、MVVM 完整代码示例（可直接复制运行）

> **说明**：本节中的 BaseView、BaseViewModel 等基类仅为**架构示意**，用于说明 MVVM 分层与依赖关系。实际开发时，项目可能采用 Provider、Riverpod、GetX 等状态管理方案，**不一定会实现或使用这些基类**，请根据团队技术选型灵活调整。

### 9.1 foundation — BaseViewModel（示例）

```dart
// foundation/base/base_view_model.dart
...
### 9.2 business/home/model — HomeModel

```dart
// business/home/model/home_model.dart
class HomeModel {
  final String title;
...
### 9.3 business/home/repository — HomeRepository

```dart
// business/home/repository/home_repository.dart
...
### 9.4 business/home/view_model — HomeViewModel

```dart
// business/home/view_model/home_view_model.dart
...
### 9.5 business/home/page — HomePage

```dart
// business/home/page/home_page.dart
        ...
        body: Consumer<HomeViewModel>(
          builder: (_, vm, __) {
            ...
            return SingleChildScrollView(
              child: Column(
                children: [
                  if (vm.model != null) HomeItemView(model: vm.model!),
                ],
              ),
            );
          },
...
### 9.6 子模块示例结构（user/profile）

与 home 模块结构相同，按 page、view、view_model、model、repository 组织。
...
本文档为团队 Flutter 项目唯一架构与规范标准，适用于新项目开发、老项目重构、新人培训、架构评审、Code Review。所有业务必须严格遵循，保证项目长期可维护、可扩展、可迭代。

文档中的 BaseView、BaseViewModel 等 MVVM 相关代码仅为**示例说明**，实际项目中可根据所选状态管理方案（Provider、Riverpod、GetX 等）自行决定是否引入基类，无需强制实现。
